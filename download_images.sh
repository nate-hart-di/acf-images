...
COOKIE_FILE="$INPUT_DIR/.cookies.txt"
...
# Function to extract Chrome cookies for a domain
extract_chrome_cookies() {
  local domain="$1"
  local cookie_db="$HOME/Library/Application Support/Google/Chrome/Default/Cookies"

  # Check if Chrome cookies database exists
  if [ ! -f "$cookie_db" ]; then
    debug "Chrome cookies database not found at: $cookie_db"
    return 1
  fi

  debug "Extracting Chrome cookies for domain: $domain"

  # Chrome cookies are in SQLite format, need to query them
  # Create a temporary copy (Chrome locks the original)
  local temp_db="/tmp/chrome_cookies_$$"
  cp "$cookie_db" "$temp_db" 2>/dev/null || {
    debug "Failed to copy Chrome cookies database"
    return 1
  }

  # Extract cookies using sqlite3 (macOS built-in)
  # Netscape cookie format for wget compatibility
  {
    echo "# Netscape HTTP Cookie File"
    echo "# This file was generated by acf-images. Edit at your own risk."
    echo ""
    sqlite3 "$temp_db" "SELECT host_key, 'TRUE', path, CASE is_secure WHEN 1 THEN 'TRUE' ELSE 'FALSE' END, expires_utc, name, value FROM cookies WHERE host_key LIKE '%${domain}%' OR host_key LIKE '.${domain}%';" 2>/dev/null | \
    awk -F'|' '{
      # Convert Chrome timestamp (microseconds since 1601) to Unix timestamp
      expires = ($5 - 11644473600000000) / 1000000
  ...
  if [ -s "$COOKIE_FILE" ]; then
    local cookie_count=$(grep -cv '^#' "$COOKIE_FILE" || echo "0")
    debug "Extracted $cookie_count cookies for $domain"
    return 0
  else
    debug "No cookies found for domain: $domain"
    rm -f "$COOKIE_FILE"
    return 1
  fi
}
...
if [ $# -ge 1 ] && [[ "$1" =~ ^https?:// ]]; then
  ...
  # Extract domain from URL for cookie extraction
  DOMAIN=$(echo "$INPUT_URL" | sed -E 's|^https?://([^/]+).*|\1|')
  debug "Extracted domain: $DOMAIN"

  # Automatically extract Chrome cookies for this domain
  if extract_chrome_cookies "$DOMAIN"; then
    print "  ✓ Using Chrome cookies for authentication"
    WGET_COOKIES="--load-cookies $COOKIE_FILE"
  else
    debug "No Chrome cookies available, proceeding without authentication"
    WGET_COOKIES=""
  fi
...
download_image() {
  ...
  # Extract domain from image URL and get cookies if needed
  local img_domain=$(echo "$url" | sed -E 's|^https?://([^/]+).*|\1|')
  local img_cookies=""
  if [ -n "$img_domain" ] && [ ! -f "$COOKIE_FILE" ]; then
    # Try to extract cookies for this image domain (might be different from HTML domain)
    if extract_chrome_cookies "$img_domain" 2>/dev/null; then
      img_cookies="--load-cookies $COOKIE_FILE"
      debug "Using cookies for image domain: $img_domain"
    fi
